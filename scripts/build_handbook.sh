...
# Each chapter starts on a new page. We rely on CSS for h1 page breaks AND
# inject an explicit page-break div before every chapter so Chromium always
# honours it (some print engines ignore page-break-before on the first h1
# inside a flow).
for f in "${CHAPTERS[@]}"; do
  name="$(basename "$f")"
  echo "Adding $name"
